<template>
  <div>
    <app-main-widget
      ...
      v-if="showWidget"
      :fromExitPopup="fromExitPopup"
      :script_id="script_id"
    ></app-main-widget>
    <app-exit-popup
      :data="appData"
      :exitPopUpdata="exitPopUpData"
      v-if="showExitPopUp"
      :script_id="script_id"
    ></app-exit-popup>
  </div>
...
export default {
  ...
  computed: {
     ...
     */
    myApiId() {
      this.script_id = document
        .querySelector('script[data-name="_emv"][data-token]')
  ...
  methods: {
    ...
    checkDomainName(dataresponse) {
      ...
      if (dataresponse.is_active) {
        ...
        if (
          dataresponse.sticky_reviews &&
          dataresponse.sticky_reviews.data.length
        ) {
