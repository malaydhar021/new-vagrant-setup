<template>
  <div>
    <transition name="fade">
      <app-rounded v-if="selectedComponent === 100"
                    :data="appData"
                    :counter="counter"
                    v-bind:key="counter"
                    :isBranded="isBranded"
                    ...
                    @startIteration="reStartIteration()"  v-cloak></app-rounded>
      <app-squared v-if="selectedComponent === 101"
                    :data="appData"
                    :counter="counter"
                    v-bind:key="counter"
                    :isBranded="isBranded"
                    ...
                    @iterationStopped="selectedComponent = ''" 
                    @iterationPaused="stopIteration()"
                    @startIteration="reStartIteration()" v-cloak></app-squared>
      <app-square-elevated v-if="selectedComponent === 102"
                    :data="appData"
                    :counter="counter"
                    v-bind:key="counter"
                    :isBranded="isBranded"
                    ...
                    @iterationStopped="selectedComponent = ''" 
                    @iterationPaused="stopIteration()"
                    @startIteration="reStartIteration()" v-cloak></app-square-elevated>
      <app-tear-drop v-if="selectedComponent === 103"
                    :data="appData"
                    :counter="counter"
                    v-bind:key="counter"
                    :isBranded="isBranded"
                    ...
                    @iterationStopped="selectedComponent = ''" 
                    @iterationPaused="stopIteration()"
                    @startIteration="reStartIteration()" v-cloak></app-tear-drop>
      <app-tear-drop-elevated v-if="selectedComponent === 104"
                    :data="appData"
                    :counter="counter"
                    v-bind:key="counter"
                    :isBranded="isBranded"
                    ...
                    @iterationStopped="selectedComponent = ''"
                    @iterationPaused="stopIteration()"
                    @startIteration="reStartIteration()" v-cloak></app-tear-drop-elevated>
    </transition>
  </div>
</template>
...
export default {
    ...
    methods: {
        ...
        showPopUps () {
            ...
            let intervalTime = vm.delayInBetweenSr > vm.srStayTime ? vm.delayInBetweenSr : vm.delayInBetweenSr + vm.srStayTime
        ...
        getNewPageData (loop) {
            ...
            if (vm.curPageNo < vm.lastPageNo) {
                ...
                vm.curPageNo = 1
                vm.fromExitPopup ? vm.getPopupReviews() : vm.getReviews()
            } else if (vm.curPageNo === vm.lastPageNo && !loop && vm.counter === vm.stickyData.length) { 
